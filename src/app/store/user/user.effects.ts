  ...
  signInEmail: Observable<Action> = createEffect(() =>
    this.actions.pipe(
      ...
      switchMap(credentials =>
        this.httpClient.post<UserResponse>(`${environment.url}/api/v1/Account/Login`, credentials)
          .pipe(
            tap((response: UserResponse) => {
              localStorage.setItem('token', response.token);
              this.router.navigate(['/']);
  ...
  init: Observable<Action> = createEffect(() =>
    this.actions.pipe(
      ...
      switchMap(token => {
        ...
        if (token) {
          return this.httpClient.get<UserResponse>(`${environment.url}/api/v1/Account`)
            .pipe(
